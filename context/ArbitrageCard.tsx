    ...
    <div className={`bg-stone-800 border rounded-lg shadow-lg overflow-hidden flex flex-col transition-all duration-200 ${isSelected ? 'border-amber-500 ring-2 ring-amber-500/50' : 'border-stone-700'}`}>
      <div className="p-4 cursor-pointer hover:bg-stone-700/50 flex-grow relative" onClick={onSelect}>
        ...
        <div className="flex justify-between items-start">
            <h3 className="text-md font-bold text-amber-400 mb-2 pr-10">{safeRenderText(card.title)}</h3>
            <span className="text-xs font-medium bg-amber-900 text-amber-300 px-2 py-1 rounded-full flex-shrink-0 ml-2">{safeRenderText(card.platform)}</span>
        </div>
        <p className="text-sm text-stone-400 line-clamp-2">{safeRenderText(card.description)}</p>
      </div>
