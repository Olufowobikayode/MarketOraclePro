    ...
    try {
        ...
        if (data.error) {
            ...
            if (
                ...
                throw new Error("❌ Invalid API Key: Google cannot recognize this key.");
            }
            ...
            if (
                ...
                throw new Error("⚠️ Quota exceeded or billing issue with this API Key.");
            }
        ...
        if (!data.candidates || data.candidates.length === 0) {
            throw new Error(
                "⚠️ The key seems valid but did not generate any content. Check API permissions."
            );
        }
